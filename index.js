    ...
    for (const [guildId, settings] of Object.entries(guilds)) {
      if (settings && typeof settings === "object") {
        ...
        guildSettings.set(guildId, {
          ...
          slotsChannelId: typeof settings.slotsChannelId === "string" ? settings.slotsChannelId : legacy,
...
function dashboardEmbed(guild) {
  ...
  return new EmbedBuilder()
    ...
    .setColor(0x2b2d31)
    .setFooter({ text: "Mykonos Shop 2k26 • Dashboard v2.7.0" });
}
...
function gameDashboardEmbed(guild, game) {
  ...
  return embed.setFooter({ text: "Mykonos Shop 2k26 • Dashboard v2.7.0" });
}
...
async function registerSlashCommands() {
  ...
  for (const guild of client.guilds.cache.values()) {
    await rest.put(Routes.applicationGuildCommands(applicationId, guild.id), { body: [gamesCommand, maintenanceCommand] });
...
client.once("clientReady", async () => {
  console.log("🚀 BUILD: Mykonos-DM-Bot v2.7.0 — MAINTENANCE + STRICT GAME CHANNELS");
  ...
  try {
    if (interaction.isChatInputCommand() && interaction.commandName === "maintenance") {
      ...
      if (!interaction.guild || !canManageGames(interaction.member, interaction.guild)) {
        await interaction.reply({ content: "❌ Μόνο Server Owner / Administrator / Manage Server μπορεί να αλλάξει το Maintenance Mode.", ephemeral: true });
        return;
      }
    ...
    if (!interaction.guild || !canManageGames(interaction.member, interaction.guild)) {
      if (interaction.isRepliable()) await interaction.reply({ content: "❌ Δεν έχεις permission για το Games Dashboard.", ephemeral: true }).catch(() => {});
      return;
    }
    ...
    if (interaction.isChannelSelectMenu() && interaction.customId.startsWith("games:set-channel:")) {
      ...
      if (!perms || !perms.has([PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageMessages])) {
        await interaction.reply({
          content: "❌ Για strict game channel το bot χρειάζεται **View Channel, Send Messages και Manage Messages** σε αυτό το κανάλι.",
          ephemeral: true,
        });
        return;
      }
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith("games:config-submit:")) {
      ...
      if (!validRate(winRate)) {
        await interaction.reply({ content: "❌ Το Winning rate πρέπει να είναι αριθμός από **0 έως 100**.", ephemeral: true });
        return;
      }
      if (!validCooldownHours(cooldownHours)) {
        await interaction.reply({ content: "❌ Το cooldown πρέπει να είναι από **0 έως 168 ώρες**.", ephemeral: true });
        return;
      }
      const current = guildSettings.get(interaction.guild.id) || {};
      ...
      if (game === "slots") {
        ...
        if (!validRate(jackpotRate) || jackpotRate > winRate) {
          await interaction.reply({ content: "❌ Το Jackpot % πρέπει να είναι από **0 έως το Winning rate**.", ephemeral: true });
          return;
        }
