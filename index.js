...
function dashboardEmbed(guild) {
  ...
  return new EmbedBuilder()
    ...
    .setColor(0x2b2d31)
    .setFooter({ text: "Mykonos Shop 2k26 • Dashboard v2.6.0" });
}
...
function gameDashboardEmbed(guild, game) {
  ...
  return embed.setFooter({ text: "Mykonos Shop 2k26 • Dashboard v2.6.0" });
}
...
client.once("clientReady", async () => {
  console.log("🚀 BUILD: Mykonos-DM-Bot v2.6.0 — ADVANCED GAME SETTINGS");
...
client.on("messageCreate", async (message) => {
  try {
    ...
    if (lower === SLOTS_COMMAND) {
      const gameChannel = await requireGameChannel(message, "slots");
      if (!gameChannel) return;
