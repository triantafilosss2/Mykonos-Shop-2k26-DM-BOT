...
const guildSettings = new Map(); // guildId -> { spinChannelId, scratchChannelId, numberChannelId }
    ...
    for (const [guildId, settings] of Object.entries(guilds)) {
      if (settings && typeof settings === "object") {
        ...
        guildSettings.set(guildId, {
          ...
          scratchChannelId: typeof settings.scratchChannelId === "string" ? settings.scratchChannelId : legacy,
          numberChannelId: typeof settings.numberChannelId === "string" ? settings.numberChannelId : legacy,
        });
      }
...
async function requireGameChannel(message, game) {
  ...
  if (!channel) {
    const labels = { spin: "Spin", scratch: "Scratch", number: "Guess Number" };
    await message.reply(
...
function dashboardEmbed(guild) {
  ...
  const spinText = settings.spinChannelId ? `<#${settings.spinChannelId}>` : "❌ Δεν έχει οριστεί";
  const scratchText = settings.scratchChannelId ? `<#${settings.scratchChannelId}>` : "❌ Δεν έχει οριστεί";
  const numberText = settings.numberChannelId ? `<#${settings.numberChannelId}>` : "❌ Δεν έχει οριστεί";

  return new EmbedBuilder()
    .setTitle("🎮 Mykonos Shop 2k26 — Games Dashboard")
    .setDescription(
      "**Κάθε παιχνίδι έχει δικό του κανάλι.**\n" +
      "Χρησιμοποίησε τα 3 dropdowns από κάτω για να ορίσεις πού θα πηγαίνει το κάθε game.\n\n" +
      "Οι εντολές μπορούν να γράφονται από οποιοδήποτε text channel."
    )
    .addFields(
      {
        name: "🎡 Spin",
        value:
          `**Κανάλι:** ${spinText}\n` +
          "`!spin` • **18%** πιθανότητα νίκης • **3 ώρες cooldown**",
        inline: false,
      },
      {
        name: "🎟️ Scratch",
        value:
          `**Κανάλι:** ${scratchText}\n` +
          "`!scratch` • αριθμός **1–1000** • jackpot **777** • **3 ώρες cooldown**",
        inline: false,
      },
      {
        name: "🎯 Guess Number",
        value:
          `**Κανάλι:** ${numberText}\n` +
          "`!setnumber <αριθμός>` • χωρίς cooldown • το command διαγράφεται\n" +
          "Οι παίκτες γράφουν απλά τον αριθμό τους στο συγκεκριμένο κανάλι.",
        inline: false,
      }
    )
    .setColor(0x2b2d31)
    .setFooter({ text: "Mykonos Shop 2k26 • Dashboard v2.4.0" });
}
...
function dashboardComponents() {
  const spinRow = new ActionRowBuilder().addComponents(
    makeChannelSelect("games:set-spin-channel", "🎡 SPIN — επίλεξε κανάλι")
  );
  const scratchRow = new ActionRowBuilder().addComponents(
    makeChannelSelect("games:set-scratch-channel", "🎟️ SCRATCH — επίλεξε κανάλι")
  );
  const numberRow = new ActionRowBuilder().addComponents(
    makeChannelSelect("games:set-number-channel", "🎯 GUESS NUMBER — επίλεξε κανάλι")
  );
  ...
  return [spinRow, scratchRow, numberRow, new ActionRowBuilder().addComponents(refresh)];
}
...
client.once("clientReady", async () => {
  console.log("🚀 BUILD: Mykonos-DM-Bot v2.4.0 — MULTI-CHANNEL DASHBOARD");
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    const channelSettingByCustomId = {
      "games:set-spin-channel": "spinChannelId",
      "games:set-scratch-channel": "scratchChannelId",
      "games:set-number-channel": "numberChannelId",
    };

    if (interaction.isChannelSelectMenu() && channelSettingByCustomId[interaction.customId]) {
      ...
      guildSettings.set(interaction.guild.id, {
        ...current,
        [channelSettingByCustomId[interaction.customId]]: channelId,
      });
      saveSettings();
...
client.on("messageCreate", async (message) => {
  try {
    ...
    if (lower === SPIN_COMMAND) {
      const gameChannel = await requireGameChannel(message, "spin");
      if (!gameChannel) return;
    ...
    if (lower === SCRATCH_COMMAND) {
      const gameChannel = await requireGameChannel(message, "scratch");
      if (!gameChannel) return;
